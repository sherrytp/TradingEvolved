...
api_key_id = 'PKNK0VETPAXXFXPWMK3R'
api_secret = 'NE5kvstn3ho02ZIjCnCXrFe5mJt5jv5LklrEAl9A'
...
"""
This algorithm may buy stocks during a 45 minute period each day, starting 15 minutes after market open. 
(The first 15 minutes are avoided, as the high volatility can lead to poor performance.) 
The signals it uses to determine if it should buy a stock are if it has gained 4% from the previous day's close, 
is above the highest point seen during the first 15 minutes of trading, and if its MACD is positive and increasing. 
...
those positions will be liquidated at market. (Potentially holding positions overnight goes against the 
concept of risk that the algorithm uses, and must be avoided for it to be effective.)
class BuyDoji:
    # Buy a stock when a doji candle forms
    def __init__(self):
        self.alpaca = tradeapi.REST(api_key_id, api_secret, base_url, api_version='v2')
    ...
    def run(self):
        ...
        async def on_minute(conn, channel, bar):
            ...
            # TODO : Take profit
        # Connect to get streaming market data, StreamConn('Polygon Key Here')???
        conn = StreamConn(api_key_id, api_secret, 'wss://alpaca.socket.polygon.io/stocks')
...
def get_1000m_history_data(symbols, start, end):
    ...
    for symbol in symbols:
        minute_history[symbol] = api.polygon.historic_agg_v2(
            # historic_agg(self, size, symbol, _from=None, to=None, limit=None)
            symbol=symbol, multiplier=1, timespan="minute", _from=start, to=end, limit=1000
        ).df
        c += 1
...
def get_tickers():
    ...
    return [ticker for ticker in tickers if (
            ...
            min_share_price <= ticker.lastTrade['p'] <= max_share_price and
...
def run(tickers, market_open_dt, market_close_dt):
    ...
    conn = tradeapi.StreamConn(key_id=api_key_id, secret_key=api_secret, data_stream='alpacadatav1')
    ...
    minute_history = get_1000m_history_data(symbols, market_open_dt, market_close_dt)
